...
* Необходимо реализовать тип `MatrixException`, который хранит информацию о причине сбоя.
...
* Если команда не может быть выполнена (например, из-за несоответствия размеров матриц в регистрах),
  значения регистров не должны изменяться (строгая гарантия исключений).

### Команда `load`
Команда `load <reg> <fname>` загружает матрицу из файла с именем `<fname>` в регистр `<reg>`.
Гарантируется, что `<fname>` не содержит пробельных символов, а соответствующий файл
существует и его формат корректен.
